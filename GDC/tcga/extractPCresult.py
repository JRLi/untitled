...
def args_parse():
    ...
    parser.add_argument('-g', '--gdc', type=str, help='GDC project, such as BRCA, LUAD, or LUSC')
...
def main(argv=None):
    if argv is None:
        ...
        gdc = 'TCGA-' + argv.gdc + '_'
        list_p = [x for x in os.listdir(argv.p_value) if gdc in x]
        time_1 = datetime.datetime.now()
        prepare_output_dir('./result')
        print('[{}]\tExtract result with {}: {}\tmode:{}'.format(argv.gdc, argv.mode, argv.threshold, argv.snv))
        with open('./result/exp_lincs_snv_{}_{}_{}.txt'.format(argv.gdc, argv.mode, argv.threshold), 'w') as outFile:
