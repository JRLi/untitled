...
def mlp(X, y):
    model = Sequential()
    model.add(Dense(units=256, input_dim=len(X[0]), kernel_initializer='normal', activation="relu"))    # hidden layer 256
    model.add(Dense(units=len(set(y)), kernel_initializer='normal', activation='softmax'))
