...
def fs_rfe(df_in, ss_label, f_n, tp=0, st=1.0):
    dfx = df_in.copy()
    if len(dfx.columns) > f_n:
        es1 = Ridge(alpha=0.1, random_state=0)
        es2 = Lasso(alpha=0.1, random_state=0)
        es3 = SVR(C=1.0, epsilon=0.2, kernel='linear', cache_size=3000)
        ch = {0: es1, 1: es2, 2: es3}
        select = RFE(ch.get(tp, es1), step=st, n_features_to_select=f_n)
...
def mvc(df_in, path_o, fn=0, st=1.0):
    ...
    if fn != 0:
        ...
        for tp in [0, 1, 2]:
            ...
            tr_x, feature_l = fs_rfe(train_x, train_y, fn, tp, st)
