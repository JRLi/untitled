...
for file_name in file_list:
    ...
    train_df, test_df = train_test_split(df, train_size=0.67, random_state=1)
    lasso_reg = linear_model.Lasso(alpha=alpha)
    lasso_reg.fit(train_df[features], train_df[target])

    test_y_predict = lasso_reg.predict(X=test_df[features])
    train_y_predict = lasso_reg.predict(X=train_df[features])
